 ...
 *
 * A share stores three images under one stem:
 ...
 *   frames/<id>-back.png  the QR side, so the shared link can flip too
 *
...
export async function uploadFrame(
  ...
  const [passResult] = await Promise.all([
    ...
    back ? put(`frames/${id}-back.png`, back) : Promise.resolve(null),
  ]);
